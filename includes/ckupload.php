<? 
	...
	$url = PATH_ROOT.'img/uploads/ckeditor/'.time()."_".$_FILES['upload']['name'];
	
	$url_aux = substr($url, strlen(PATH_ROOT) - 1);
		
    if (($_FILES['upload'] == "none") OR (empty($_FILES['upload']['name'])) )
    {
       $message = "No file uploaded.";
    }
    else if(file_exists(PATH_ROOT.'images/uploads/'.$_FILES['upload']['name']))
    {
